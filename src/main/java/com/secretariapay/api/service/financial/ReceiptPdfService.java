...
public class ReceiptPdfService {
    ...
    public byte[] generateReceiptPdf(UUID receiptId) {
        ...
        ChargeCategory category = classificationService.resolveCategory(anchor.getCharge());
        ...
        try (PDDocument document = new PDDocument(); ByteArrayOutputStream output = new ByteArrayOutputStream()) {
            ...
            try (PDPageContentStream content = new PDPageContentStream(document, page)) {
                draw(document, page, content, anchor, receipts, student, category);
            }
            document.save(output);
            return output.toByteArray();
        } catch (Exception exception) {
            throw new IllegalStateException("Não foi possível gerar o documento financeiro.", exception);
        }
    }

    private void draw(PDDocument doc, PDPage page, PDPageContentStream c, Receipt anchor, List<Receipt> receipts,
                      Student student, ChargeCategory category) throws Exception {
        float w = page.getMediaBox().getWidth(), h = page.getMediaBox().getHeight(), m = 18;
        String documentTitle = category == ChargeCategory.TUITION
                ? "BORDERÔ DE PROPINAS"
                : category == ChargeCategory.ACADEMIC_SERVICE
                ? "HISTÓRICO DE SERVIÇOS ACADÉMICOS"
                : "COMPROVATIVO DE PAGAMENTOS";
        String detailTitle = category == ChargeCategory.TUITION
                ? "HISTÓRICO DE PAGAMENTOS DE PROPINAS"
                : category == ChargeCategory.ACADEMIC_SERVICE
        ...
        center(c, documentTitle, 20, w / 2, h - 54, NAVY, true);
        line(c, GOLD, w / 2 - 102, h - 66, w / 2 + 102, h - 66, 1);
        center(c, "Emitido em: " + DATE_TIME.format(anchor.getIssuedAt()) + "  |  Documento Nº " + anchor.getReceiptCode(), 9, w / 2, h - 83, NAVY, false);

        float infoY = h - 108;
        fill(c, LIGHT, m + 1, infoY - 69, w - 2 * m - 2, 69);
        stroke(c, NAVY, m + 1, infoY - 69, w - 2 * m - 2, 69, .7f);
        AcademicClass ac = student.getAcademicClass();
        Course course = ac == null ? null : ac.getCourse();
        pair(c, "NOME", student.getFullName(), 30, infoY - 18);
        pair(c, "MATRÍCULA", student.getStudentNumber(), 30, infoY - 37);
        pair(c, "CURSO", course == null ? "-" : course.getName(), 30, infoY - 56);
        pair(c, "ANO ACADÉMICO", ac == null ? "-" : ac.getAcademicYear(), 320, infoY - 18);
        pair(c, "TURMA", ac == null ? "-" : ac.getName(), 320, infoY - 37);
        pair(c, "TELEFONE", mask(student.getPhone()), 320, infoY - 56);
        pair(c, "DOCUMENTO", mask(student.getDocumentNumber()), 575, infoY - 27);
        pair(c, "E-MAIL", student.getEmail(), 575, infoY - 50);

        float titleY = infoY - 82;
        fill(c, NAVY, m + 1, titleY - 22, w - 2 * m - 2, 22);
        center(c, detailTitle, 11, w / 2, titleY - 15, Color.WHITE, true);
        float headerY = titleY - 26;
        fill(c, NAVY, m + 1, headerY - 25, w - 2 * m - 2, 25);
        String[] heads = {"Nº", "DESCRIÇÃO", "REF. PERÍODO", "REFERÊNCIA / GUIA", "DATA DO PAGAMENTO", "ESTADO", "VALOR BRUTO", "DESCONTOS", "JUROS / MULTA", "VALOR LÍQUIDO"};
        ...
        for (Receipt receipt : receipts) {
            ...
            fittedText(c, safe(charge.getDescription()), 8, 6.4f, 52, y, 120, NAVY, false);
            fittedText(c, safe(charge.getReferenceMonth()), 8, 6.2f, 178, y, 62, NAVY, false);
            fittedText(c, safe(charge.getChargeCode()), 7.4f, 5.8f, 246, y, 88, NAVY, false);
            fittedText(c, charge.getPaidAt() == null ? "-" : DATE_TIME.format(charge.getPaidAt()), 7.3f, 6.1f, 340, y, 91, NAVY, false);
            fittedText(c, "PAGAMENTO CONFIRMADO", 6.5f, 5.6f, 438, y, 104, GREEN, true);
            text(c, money(charge.getAmount()), 8, 552, y, NAVY, false);
            text(c, money(charge.getDiscountAmount()), 8, 620, y, NAVY, false);
            BigDecimal fee = nz(charge.getFineAmount()).add(nz(charge.getInterestAmount()));
            text(c, money(fee), 8, 688, y, NAVY, false);
            text(c, money(charge.getTotalAmount()), 8, 756, y, GREEN, true);
            line(c, new Color(210, 218, 232), m + 2, y - 12, w - m - 2, y - 12, .4f);
            y -= 37;
            gross = gross.add(nz(charge.getAmount()));
            disc = disc.add(nz(charge.getDiscountAmount()));
            fees = fees.add(fee);
            total = total.add(nz(charge.getTotalAmount()));
        }

        line(c, NAVY, m + 1, y + 14, w - m - 1, y + 14, .8f);
        text(c, "Nº TOTAL DE REGISTOS:  " + receipts.size(), 8, 28, y - 2, NAVY, true);
        text(c, "SUBTOTAL:", 8, 314, y - 2, NAVY, true);
        text(c, money(gross), 8, 464, y - 2, NAVY, false);
        text(c, "ACRÉSCIMOS:", 8, 314, y - 17, NAVY, true);
        text(c, money(fees), 8, 464, y - 17, new Color(20, 85, 180), false);
        text(c, "DESCONTOS:", 8, 314, y - 32, NAVY, true);
        text(c, money(disc), 8, 464, y - 32, Color.RED, false);
        fill(c, NAVY, 302, y - 56, 210, 20);
        text(c, "TOTAL LÍQUIDO:", 8, 314, y - 50, Color.WHITE, true);
        text(c, money(total) + " KZ", 10, 432, y - 50, Color.WHITE, true);

        float boxY = 43, boxH = 105;
        stroke(c, NAVY, m + 1, boxY, w - 2 * m - 2, boxH, .7f);
        String hash = authenticityService.hash(anchor);
        String url = anchor.getValidationUrl();
        if (url == null || !url.contains("hash=")) {
            url = "https://secretariapay-api.paixaoangola.com/api/v1/public/receipts/validate/" + anchor.getReceiptCode() + "/authentic?hash=" + hash;
        }
        BufferedImage qr = createQr(url);
        c.drawImage(LosslessFactory.createFromImage(doc, qr), 31, 54, 82, 82);
        text(c, "V", 25, 132, 111, GREEN, true);
        text(c, "DOCUMENTO VÁLIDO", 9, 165, 117, GREEN, true);
        text(c, category == ChargeCategory.TUITION ? "Este documento comprova apenas propinas recebidas" : "Este documento comprova apenas serviços académicos pagos", 8, 165, 101, NAVY, false);
        text(c, "e registados pela tesouraria do IMETRO.", 8, 165, 87, NAVY, false);
        text(c, "Validação pública por QR Code", 8, 165, 71, GREEN, true);
        text(c, "Código de verificação: " + authenticityService.shortHash(anchor) + "-IMETRO", 7.5f, 165, 57, NAVY, false);
        line(c, new Color(170, 180, 195), 414, 53, 414, 136, .6f);
        center(c, "RESPONSÁVEL / TESOURARIA", 8, 535, 118, NAVY, true);
        center(c, "Assinado digitalmente", 11, 535, 91, new Color(39, 54, 205), false);
        line(c, NAVY, 462, 78, 608, 78, .5f);
        center(c, "Tesouraria IMETRO", 8, 535, 62, NAVY, false);
        center(c, "SELO DIGITAL", 8, 700, 112, new Color(130, 165, 225), true);
        stroke(c, new Color(130, 165, 225), 654, 56, 92, 72, 1.2f);
        center(c, "IMETRO", 16, 700, 84, new Color(130, 165, 225), true);
        center(c, "LIQUIDADO", 7, 700, 68, GREEN, true);
        line(c, NAVY, m + 1, 35, w - m - 1, 35, .7f);
        center(c, "Documento emitido eletronicamente pelo SecretáriaPay Académico - IMETRO", 8, w / 2, 24, NAVY, false);
        center(c, "Propinas e serviços académicos são apresentados em documentos separados.", 7.5f, w / 2, 14, NAVY, false);
    }
    ...
    private BufferedImage createQr(String value) throws Exception {
        ...
        BitMatrix matrix = new QRCodeWriter().encode(value, BarcodeFormat.QR_CODE, 500, 500, hints);
    ...
    private void image(PDDocument d, PDPageContentStream c, String path, float x, float y, float w, float h) {
        try {
            ...
            if (r.exists()) {
                var i = org.apache.pdfbox.pdmodel.graphics.image.PDImageXObject.createFromByteArray(d, r.getInputStream().readAllBytes(), path);
