...
public class SecretariaPayFinancialProductionGuardAspect {
    ...
    public Object protectMainFinancialFlow(ProceedingJoinPoint joinPoint) throws Throwable {
        if (shouldBlockTestPayment(joinPoint.getArgs())) {
            return Optional.of(buildInstitutionalPaymentNotice());
        }
    ...
    private boolean shouldBlockTestPayment(Object[] args) {
        if (infinitePayTestEnabled || args == null || args.length < 3) {
            return false;
        }
    ...
    private Object sanitizeResult(Object result) {
        if (!(result instanceof Optional<?> optional) || optional.isEmpty() || !(optional.get() instanceof String text)) {
            return result;
        }
    ...
    String sanitizeFinancialText(String value) {
        ...
        String sanitized = Arrays.stream(value.split("\\R", -1))
                .filter(line -> !containsAny(normalize(line),
                        "infinitepay",
                        "infinite pay",
                        "teste real brasil",
                        "pix brasil"))
