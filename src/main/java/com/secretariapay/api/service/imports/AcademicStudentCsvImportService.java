...
public class AcademicStudentCsvImportService {
    ...
    public AcademicStudentImportBatch upload(
        ...
        AcademicStudentImportBatch batch = new AcademicStudentImportBatch()
                ...
                .setNotes("Lote criado por upload CSV no painel SecretáriaPay.");

        AcademicStudentImportBatch savedBatch = importService.createBatch(batch);
        List<List<String>> records = readRecords(file);
        if (records.size() < 2) {
            throw new IllegalArgumentException("O ficheiro CSV deve conter cabeçalho e pelo menos uma linha de dados.");
        }
    ...
    private void validateFile(MultipartFile file) {
        if (file == null || file.isEmpty()) {
            throw new IllegalArgumentException("Selecione um ficheiro CSV para importar.");
        }
        ...
        if (name == null || !name.toLowerCase(Locale.ROOT).endsWith(".csv")) {
            throw new IllegalArgumentException("Formato inválido. Nesta etapa são aceites apenas ficheiros .csv.");
        }
    }
    ...
    private void requireAny(Map<String, Integer> header, String... aliases) {
        ...
        throw new IllegalArgumentException("CSV inválido: falta uma coluna obrigatória. Aceites: " + String.join(", ", aliases));
    }
