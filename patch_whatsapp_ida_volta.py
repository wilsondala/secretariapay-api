...
# 1) Substitui buyTicket para perguntar Só ida / Ida e volta
buy_ticket_pattern = r'\s*private WhatsappCommandResult buyTicket\(WhatsappSessionResponse session\) \{.*?return allowed\("BUY_TICKET", reply\.trim\(\)\);\s*\}'
buy_ticket_replacement = r'''
    ...
    private WhatsappCommandResult handleTripTypeSelection(
        ...
        if (isRoundTripIntent(normalizedMessage)) {
            ...
            return allowed(
                    "ASK_OUTBOUND_TRIP",
                    buildOutboundTripSearchPrompt(true));
        }
        ...
        if (isOneWayIntent(normalizedMessage)) {
            ...
            return allowed(
                    "ASK_OUTBOUND_TRIP",
                    buildOutboundTripSearchPrompt(false));
        }
    ...
    private WhatsappCommandResult handleOutboundTripSearchAnswer(
        ...
        if (input == null) {
            ...
            return allowed(
                    "ASK_OUTBOUND_TRIP",
                    buildOutboundTripSearchPrompt(roundTrip));
        }
        ...
        if (returnDate.isBefore(outboundDate)) {
            return allowed(
                    ...
                    """
                            ...
                            destination,
                            origin,
                            outboundDate.plusDays(1).format(DATE_FORMATTER)).trim());
        }
    ...
    private String buildReturnTripSearchPrompt(
        ...
        return """
                ...
                suggestedReturnDate.format(DATE_FORMATTER)).trim();
    }
...
text, count = re.subn(buy_ticket_pattern, buy_ticket_replacement, text, count=1, flags=re.S)
if count != 1:
    raise SystemExit("Não consegui substituir o método buyTicket. Verifique se o arquivo mudou.")

# 2) Adiciona os handlers novos dentro do fluxo PASSENGER
old_handle_block = '''            if (isBuyTicketCommand(normalizedMessage)
...
new_handle_block = old_handle_block + '''
...
if old_handle_block not in text:
    raise SystemExit("Não encontrei o bloco isBuyTicketCommand no handleCommand.")
text = text.replace(old_handle_block, new_handle_block, 1)

# 3) Melhorar parseTripSearch para aceitar "Luanda > Benguela 26/06/2026"
parse_pattern = r'\s*private TripSearchInput parseTripSearch\(String messageText\) \{.*?\n\s*private boolean isRoundTripRequest'
parse_replacement = r'''
    private record RouteExpression(String origin, String destination) {
    }
    ...
    private TripSearchInput parseTripSearch(String messageText) {
        ...
        RouteExpression routeExpression = extractRouteExpression(messageText);

        if (routeExpression != null
                && isFilled(routeExpression.origin())
                && isFilled(routeExpression.destination())) {
            return new TripSearchInput(
                    routeExpression.origin(),
                    routeExpression.destination(),
                    date,
                    returnDate);
        }
    ...
    private RouteExpression extractRouteExpression(String messageText) {
        if (messageText == null || messageText.isBlank()) {
            return null;
        }
        ...
        if (parts.length >= 2) {
            ...
            if (isFilled(origin) && isFilled(destination)) {
                return new RouteExpression(origin, destination);
            }
        }
    ...
    private String cleanRouteToken(String value) {
        ...
        return value
                ...
                .replaceAll("(?iu)\\\\b(volta|retorno|regresso|ida)\\\\b.*$", "")
                .replace(":", "")
                .replace(";", "")
                .trim();
    }

    private boolean isRoundTripRequest'''

text, count = re.subn(parse_pattern, parse_replacement, text, count=1, flags=re.S)
if count != 1:
    raise SystemExit("Não consegui substituir o parseTripSearch.")
